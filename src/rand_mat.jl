...
"""
```julia
precond::Preconditioner = create_precond_rand_inv(
    A   ::Matrix{TA},  # Matrix A 
    M   ::Matrix{TM},  # Preconditioner M of A
    uₐ  ::DataType,    # Precision at which the matrix A is applied
    uₘ  ::DataType     # Precision at which M^{-1} is applied
...
```
Same as [`create_precond_rand`](@ref), but use an explicit inversion of the 
preconditioner. 
"""
function create_precond_rand_inv(
        A::Matrix{TA},
        M::Matrix{TM},
        uₐ::DataType,
        uₘ::DataType
    ...
    ###########################################################################
    # invMₜ = inv(xconvert(Float128, A))
    ###########################################################################

    if (eltype(invMₜ) != uₘ)
        invMₘ = xconvert(uₘ, invMₜ)
    else
        invMₘ = invMₜ
    end
    ...
    Mx = function (x, do_stats::Bool)
        ...
        return invMₘ * xconvert(uₘ, x), dict;
    end
    ...
    AMx = function (x, do_stats::Bool)
        ...
        zₐ = xconvert(uₐ, invMₘ * xconvert(uₘ, x))
        # print(eltype(Aₐ), "\n")
        # print("|Az|   = ", norm(Aₐ * zₐ),"\n")
        # print("|A||z| = ", norm(abs.(Aₐ) * abs.(zₐ)),"\n")
        return Aₐ * zₐ, zₐ, dict;
    end
    ...
    MAx = function (x, do_stats::Bool)
        ...
        dict["|Avⱼ|"]   = norm(Aₐ * xconvert(uₐ, x), 2)
        dict["|A||vⱼ|"] = norm(abs.(Aₐ) * abs.(x), 2)
        return invMₘ * xconvert(uₘ, Aₐ * xconvert(uₐ, x)), dict;
    end
